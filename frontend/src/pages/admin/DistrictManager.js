...
function DistrictManager(props){
    ...
    const fetchData = () => {
        getAllDistrict(currentPage - 1, itemsPerPage, searchQuery).then(response => {
    ...
    const handleRedirectAddCategory = () => {
        history('/add-district')
    }

    const handleEditCategory = (id) => {
        history('/edit-district/' + id)
    }
                ...
                <div className="main">
                    ...
                    <div className="card">
                        <div className="card-header">
                            <h5 className="card-title">Quản lý huyện</h5>
                            <h6 className="card-subtitle text-muted"> Quản lý kho háng của TVL Post.</h6>
                        </div>
                        <div className="card-body">
                            <div id="datatables-buttons_wrapper" className="dataTables_wrapper dt-bootstrap5 no-footer"><div className="row"><div className="col-sm-12 col-md-6"><div className="dt-buttons btn-group flex-wrap">
                                <button className="btn btn-secondary buttons-copy buttons-html5" tabindex="0" aria-controls="datatables-buttons" type="button"><a onClick={handleRedirectAddCategory}>Thêm huyện</a></button>
                            </div></div>
                                <div className="col-sm-12 col-md-6"><div id="datatables-buttons_filter" className="dataTables_filter">
                                    ...
                                    <thead>
                                        <tr>
                                            <th className="sorting sorting_asc" tabindex="0" aria-controls="datatables-buttons" rowspan="1" colspan="1" style={{ width: "224px" }}  >Tên huyện</th>
                                            <th className="sorting sorting_asc" tabindex="0" aria-controls="datatables-buttons" rowspan="1" colspan="1" style={{ width: "224px" }}  >Thành phố</th>
                                            <th className="sorting" tabindex="0" aria-controls="datatables-buttons" rowspan="1" colspan="1" style={{ width: "15px" }} >Chế độ</th></tr>
                                    </thead>
                                    <tbody>
                                        {tableData?.map((item) => (
                                            <tr className="odd">
                                                <td className="dtr-control sorting_1" tabindex="0">{item?.name}</td>
                                                <td className="dtr-control sorting_1" tabindex="0">{item?.province?.name}</td>
                                                <td>
